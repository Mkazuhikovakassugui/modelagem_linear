...
Por meio dos coeficientes padronizados, entre estas variáveis acima listadas, **_lstat_** (menor status da população) com coeficiente (`r cfp[['lstat']]`) e **_dis_** (distância média ponderada dos cinco centros de emprego) com coeficiente (`r cfp[['dis']]`) **são as que exercem as maiores influências negativas sobre o preço mediano das casas**.
